...
def test_game_initialization(difficulty):
    """Tests if the game initializes correctly with different levels."""
    pygame.init()
...
def test_process_input(mock_keys, game_instance):
    ...
    game_instance.platform_object.move.assert_called_once_with(
        "left", SCREEN_WIDTH
    )
...
def test_platform_moves_right(mock_keys, game_instance):
    """Tests if the platform moves right when
